...
SERVER_PATH = str(
    pathlib.Path(__file__).parent.resolve().joinpath("./api/src/Portfolio/main.py")
)

BUILD_PATH = str(pathlib.Path(__file__).parent.resolve().joinpath("./client"))
...
def build_and_host():
    ...
    session = subprocess.Popen(
        f"exec fastapi run --port 3000 {str(SERVER_PATH)}",
        stdout=API_LOG,
        stderr=API_LOG,
        shell=True,
    )
